...
void PhysicsSystem::SetLinDamping(float d) {
	linearDamping = d;
}

float PhysicsSystem::GetLinDamping() {
...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	if (totalMass == 0) {
		return; //two static objects colliding hmm
	}
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	for (auto i = first; i != last; ++i) {
		...
		float frameLinearDamping = 1.0f - (object->GetFriction() * dt);
		...
		orientation = orientation + (Quaternion(angVel * dt * .5f, 0.0f) * orientation);
		orientation.Normalise();

		transform.SetOrientation(orientation);

		//Damping angular
		float frameAngularDamping = 1.0f - (object->GetFriction() * dt);
